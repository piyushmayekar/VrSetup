...
public class A_GasWelding : MonoBehaviour
{
    public static A_GasWelding instance;
    ...
    {
        /*
        ZeroMeterred = gasWelding.ZeroMeterred;
        ZeroMeterBlack = gasWelding.ZeroMeterBlack;

        toolToResetPosition = new List<Vector3>();
        for (int i = 0; i < toolToResetPosition.Count; i++)
        {
            toolToResetPosition.Add(gasWelding.toolToResetPosition[i]);
        }

        toolToResetRotate = new List<Vector3>();
        for (int i = 0; i < toolToResetRotate.Count; i++)
        {
            toolToResetRotate.Add(gasWelding.toolToResetRotate[i]);
        }

        toolToReset = new Transform[gasWelding.toolToReset.Length];
        for (int i = 0; i < toolToReset.Length; i++)
        {
            toolToReset[i] = gasWelding.toolToReset[i];
        }

        ppekitcolliders = new Collider[gasWelding.ppekitcolliders.Length];
        for (int i = 0; i < ppekitcolliders.Length; i++)
        {
            ppekitcolliders[i] = gasWelding.ppekitcolliders[i];
        }

        GasTablekitcolliders = new Collider[gasWelding.GasTablekitcolliders.Length];
        for (int i = 0; i < GasTablekitcolliders.Length; i++)
        {
            GasTablekitcolliders[i] = gasWelding.GasTablekitcolliders[i];
        }

        objectOutLines = new Outline[gasWelding.objectOutLines.Length];
        for (int i = 0; i < objectOutLines.Length; i++)
        {
            objectOutLines[i] = gasWelding.objectOutLines[i];
        }

        rotateNozzles = new RotateNozzle[gasWelding.rotateNozzles.Length];
        for (int i = 0; i < rotateNozzles.Length; i++)
        {
            rotateNozzles[i] = gasWelding.rotateNozzles[i];
        }

        chainStartSphere = gasWelding.chainStartSphere;
        countppekit = gasWelding.countppekit;
        countCrackTab = gasWelding.countCrackTab;
        isPipeRedConnect = gasWelding.isPipeRedConnect;
        isPipeblueConnect = gasWelding.isPipeblueConnect;
        readSteps = gasWelding.readSteps;
        stepAudioSource = gasWelding.stepAudioSource;
        creckykeyClip = gasWelding.creckykeyClip;
        blueStartpos = gasWelding.blueStartpos;
        redStartpos = gasWelding.redStartpos;
        isRedCrecking = gasWelding.isRedCrecking;
        isBlackCrecking = gasWelding.isBlackCrecking;

        finishPanel = gasWelding.finishPanel;
        setPressureRegulatorCanvas = gasWelding.setPressureRegulatorCanvas;
        LighterCanvas = gasWelding.LighterCanvas; 
        lighter_Flame = gasWelding.lighter_Flame;
        creacking_C_key1_canvas = gasWelding.creacking_C_key1_canvas;
        creacking_C_key2_canvas = gasWelding.creacking_C_key2_canvas;
        ChainHighLight = gasWelding.ChainHighLight;
        BluePipEndPoint = gasWelding.BluePipEndPoint;
        RedPipeEndPoint = gasWelding.RedPipeEndPoint;
        ParentBluePipEndPoint = gasWelding.ParentBluePipEndPoint;
        ParentRedPipeEndPoint = gasWelding.ParentRedPipeEndPoint;
        nozzelSnapPoint = gasWelding.nozzelSnapPoint;
        RedRegulator = gasWelding.RedRegulator;
        blackRegulator = gasWelding.blackRegulator;
        redPipeRop = gasWelding.redPipeRop;
        bluePipeRop = gasWelding.bluePipeRop;
        blacksmoke = gasWelding.blacksmoke;
        oxidizing_F = gasWelding.oxidizing_F;
        reduce_or_carb_F = gasWelding.reduce_or_carb_F;
        neturel_F = gasWelding.neturel_F;
        HL_flashbackred = gasWelding.HL_flashbackred;
        HL_connectorRed = gasWelding.HL_connectorRed;
        HL_R_ClipRed = gasWelding.HL_R_ClipRed;
        HL_flashbackBlack = gasWelding.HL_flashbackBlack;
        HL_connectorBlack = gasWelding.HL_connectorBlack;

        HL_R_ClipBlack = gasWelding.HL_R_ClipBlack;
        HL_T_connectorRed = gasWelding.HL_T_connectorRed;
        HL_T_ClipRed = gasWelding.HL_T_ClipRed;
        HL_T_connectorBlack = gasWelding.HL_T_connectorBlack;
        HL_T_ClipBlack = gasWelding.HL_T_ClipBlack;
    
        step8Flame = gasWelding.step8Flame;
        Step9flame = gasWelding.Step9flame;
        extraRedBol = gasWelding.extraRedBol;
        oldRedBol = gasWelding.oldRedBol;
        redRotateSprite = gasWelding.redRotateSprite;
        BlueRotatesprite = gasWelding.BlueRotatesprite;
        */
    }
    ...
    public void ConfirmSatrtbtn()
    {
        ...
        {
            readSteps.AddClickConfirmbtnEvent(Experiment15FlowManager.instance.OnNextBtnClick);
        }
        else if (SceneManager.GetActiveScene().name.Contains("14"))
        {
            readSteps.AddClickConfirmbtnEvent(Experiment14FlowManager.instance.OnNextBtnClick);
        }
    ...
    void EnableWeldingTouchCanvas()
    {
        ...
        readSteps.AddClickEventVideoPlay(3); // welding torch connector video clip animation
    ...
    public void onEnableStep_7_part3_object()
    {
        ...
        GasTablekitcolliders[15].transform.Find("Glass/Cylinder").GetComponent<MeshRenderer>().enabled = false;
        //GasTablekitcolliders[15].transform.Find("Bottle(1)").gameObject.SetActive(false);
    ...
    void Onclickbtn_s_7_confirm()
    {
        ...
        BlueRotatesprite.transform.localEulerAngles = new Vector3(0, -90, 260);
        redRotateSprite.transform.localEulerAngles = new Vector3(0, -90, 260);
    ...
    public void OnEnableNozzleObject()
    {
        ...
        GasTablekitcolliders[16].transform.Find("Glass/Cylinder").GetComponent<MeshRenderer>().enabled = false;
        //GasTablekitcolliders[16].transform.Find("Bottle(1)").gameObject.SetActive(false);
    ...
    public void CheckWelding_rot(GameObject weldingTorch)
    {
        ...
        Vector3 pos = GasTablekitcolliders[6].transform.localPosition;
        pos = weldingTorch.transform.localPosition;

        GasTablekitcolliders[6].transform.localPosition = pos;

        GasTablekitcolliders[6].GetComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezeRotation;

        GasTablekitcolliders[6].transform.localEulerAngles = weldingTorch.transform.localEulerAngles;

    }


    void OnClickBtn_WeldingStep()
    {
        if (SceneManager.GetActiveScene().name.Contains("15"))
        {
            readSteps.AddClickConfirmbtnEvent(Experiment15FlowManager.instance.OnNextBtnClick);
